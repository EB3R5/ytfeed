...
def _process_batch(
    ...
    for item in batch_items:
        _finalize_item(session, config, rag_dir, item, videos[item.video_id], results.get(item.video_id))
...
def _run_locked(
    ...
    *,
    ...
    # resolve the vault dir before any item is marked processing, so an unset
    # path fails here with a clear message instead of mid-batch
    rag_dir = require_path(config, "rag_output_dir")
    ...
    for i in range(0, len(workable), batch_size):
        ...
        results.update(_process_batch(session, config, rag_dir, batch, videos))
