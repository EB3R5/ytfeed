 ...
 *
 * The repo path is compiled in by build.sh (-DYTFEED_REPO="...") so the
 * signed binary is self-contained; YTFEED_REPO in the environment
...
#ifndef YTFEED_REPO
#error "YTFEED_REPO is not defined: build with packaging/desktop/macos/build.sh"
#endif
...
int main(void) {
    ...
    const char *repo = getenv("YTFEED_REPO");
    if (repo == NULL || repo[0] == '\0') repo = YTFEED_REPO;
    if (chdir(repo) != 0) { perror(repo); return 1; }
